...
interprete s e c d = case (head c) of 
    ...
    Ap -> case (head s) of (CLO c1 e1) -> case (head (tail s)) of VLISTA x -> (interprete [] (x:e1) c1 ((TRIPLA (tail(tail s)) e (tail c)):d))
...
e2 = "let z=2 in letrec x= 2+z and y= 2*z in x*y*z end end $"
...
g2="let f1 = lambda() 1 in f1() end $"

g3="let f1=lambda() letrec fact=lambda(x) if eq(x,1) then 1 else x*fact(x-1) and z=4 in fact(3)*z end in f1() end $"
...
k()= fin(comp_one(parser(lexi(f))))

k1()= fin(comp_one(parser(lexi(m))))

k2()= fin(comp_one(parser(lexi(n))))

k3()= fin(comp_one(parser(lexi(e))))

k4()= fin(comp_one(parser(lexi(s))))

k5()= fin(comp_one(parser(lexi(d1))))

k6()= fin(comp_one(parser(lexi(e2))))

k7()= fin(comp_one(parser(lexi(g))))

k8()= fin(comp_one(parser(lexi(g2))))

k9()= fin(comp_one(parser(lexi(g3))))
